...
<?php
	...
	//query for all user input
		...
		$template->loadTemplatefile("template1.tpl", true, true);		

		if($yearstart > $yearend || $minimalprice > $maximalprice)
		{
			$template->setCurrentBlock("VALIDATION"); //Set current block to "VALIDATION"
			...
			$template->parseCurrentBlock(); //to stop the current block
